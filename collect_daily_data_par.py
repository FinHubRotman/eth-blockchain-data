...
def write_token_data(i, date, name, sym, addr):
    ...
    if not already_written:
        with open(filename, 'a', newline="") as f:
            ...
            if i < 100:
                ...
                data_with_date = {**{'date':date}, **andreas_data(date_data)}
...
if __name__ == '__main__':
    ...
    for i in list(range(273))[:]: # go backwards in time
        date = datetime.datetime(2018,10,1)
        date = date + datetime.timedelta(days=-1*i)
        fix_i = functools.partial(write_token_data, i)
        fix_date = functools.partial(fix_i, date)
        procs = []
        for some_list in new_lists:
            t = threading.Thread(target=fix_date, args=(*some_list,))
            procs.append(t)
